"""
V3.6 顶部凹陷检测 V2 — 只分析大花色符号区域（y=112~220，排除 rank indicator）
"""
import os
...
# 大花色符号区域：排除 rank indicator（y=56~104）
# 大花色从 y≈112 开始，到 y≈220 结束
# 在 260 高度的牌中，占比 112/260=43% 到 220/260=85%
SUIT_Y_START_FRAC = 0.43
SUIT_Y_END_FRAC = 0.85
...
def detect_top_indentation(row_widths):
    """
    检测顶部凹陷：♥ 有凹陷（先增后减再增），♦ 单调递增
    返回凹陷分数：越高越像 ♥
    """
    if len(row_widths) < 10: return 0.0

    # 找到最宽行
    max_w = max(row_widths)
    if max_w == 0: return 0.0
    widest_y = row_widths.index(max_w)

    # 只分析从顶部到最宽行的部分
    if widest_y < 5: return 0.0

    top_segment = row_widths[:widest_y]

    # 检测是否有"凹陷"：先增后减再增
    # 计算局部极大值和极小值
    has_indent = False
    increasing = True
    min_after_increase = max_w

    for i in range(1, len(top_segment)):
        if increasing:
            if top_segment[i] < top_segment[i-1] - 3:  # 开始下降（凹陷）
                increasing = False
                has_indent = True
        else:
            min_after_increase = min(min_after_increase, top_segment[i])
            if top_segment[i] > top_segment[i-1] + 3:  # 再次上升
                break

    # 凹陷深度 = (峰值 - 谷值) / 峰值
    if has_indent and widest_y > 5:
        peak = max(top_segment[:5]) if len(top_segment) >= 5 else max(top_segment[:len(top_segment)//2])
        valley = min(top_segment[5:widest_y]) if widest_y > 5 else min(top_segment)
        indent_depth = (peak - valley) / max(peak, 1)
        return indent_depth
...
def classify_suit_v6(mask, row_widths, reg_w, reg_h, is_red):
    """V3.6 顶部凹陷检测 V2"""
    total_px = sum(row_widths)
    if total_px < 5: return "?"

    max_width = max(row_widths)
    widest_y = row_widths.index(max_width)
    wp = widest_y / reg_h
    ...
    top_indent = detect_top_indentation(row_widths)

    if is_red:
        # ♥ HEART: 顶部凹陷明显
        # ♦ DIAMOND: 顶部无凹陷，单调递增
        heart_score = 0.0
        if top_indent > 0.30: heart_score += 4.0  # 凹陷明显
        elif top_indent > 0.20: heart_score += 2.0
        if 0.40 < com_y < 0.70: heart_score += 1.5  # 重心居中
        if total_px > 0 and abs(ts - bs) / total_px < 0.40: heart_score += 1.0  # 对称
        return "h" if heart_score > 3.5 else "d"
    else:
        # ♠ SPADE: 顶部分散 + 底部重
        spade_score = 0.0
        if top_x_std > 14: spade_score += 4.0
        elif top_x_std > 10: spade_score += 2.0
        if wp > 0.60: spade_score += 2.0
...
def analyze_suit(pixels, w, h, x1, y1, x2, y2):
    ...
    suit = classify_suit_v6(mask, row_widths, reg_w, reg_h, is_red)
