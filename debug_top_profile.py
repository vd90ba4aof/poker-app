...
def analyze_top_profile(pixels, sw, sh, x1, y1, x2, y2, is_red, label):
    card_h = y2 - y1
    suit_y1 = y1 + int(card_h * 0.35)
    suit_y2 = y1 + int(card_h * 0.90)
    reg_w = x2 - x1; reg_h = suit_y2 - suit_y1

    row_widths = [0] * reg_h
    for y in range(reg_h):
        for x in range(reg_w):
            p = get_pixel(pixels, sw, x1 + x, suit_y1 + y)
    ...
    print(f"\n{label} (max={max_w}):")
    # 打印前 20 行
    for y in range(min(20, reg_h)):
        bar = "█" * (row_widths[y] // 2)
        ratio = row_widths[y] / max_w if max_w > 0 else 0
        print(f"  y={y:2d}: {row_widths[y]:3d} ({ratio:.2f}) {bar}")

cases = [
    ("Screenshot_2026-06-12-22-45-58-77_1d58bed7e226cc412b0128cc8fb4cf50.jpg", 180, 325, 1030, 1290, True, "3♥"),
    ("Screenshot_2026-06-13-02-20-54-87_1d58bed7e226cc412b0128cc8fb4cf50.jpg", 325, 460, 1030, 1290, True, "8♥"),
    ("Screenshot_2026-06-13-02-24-21-01_1d58bed7e226cc412b0128cc8fb4cf50.jpg", 180, 325, 1030, 1290, True, "9♦"),
    ("Screenshot_2026-06-13-02-26-11-07_1d58bed7e226cc412b0128cc8fb4cf50.jpg", 460, 595, 1030, 1290, True, "9♥"),
    ("Screenshot_2026-06-13-02-26-11-07_1d58bed7e226cc412b0128cc8fb4cf50.jpg", 180, 325, 1030, 1290, True, "10♥"),
]
...
for fn, bx1, bx2, by1, by2, is_red, label in cases:
    ...
    analyze_top_profile(pixels, sw, sh, sx1, sy1, sx2, sy2, is_red, label)
