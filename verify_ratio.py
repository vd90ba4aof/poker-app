"""Ratio方案: middle/peak + lower/peak，完全不受rank位置影响"""
...
def test_all(PATH, test_cards, label):
    ...
    for name, x1, y1, x2, y2, is_hand, exp in test_cards:
        ...
        if peak_w == 0:
            print(f"  {name}: peak=0 → ? ❌"); continue
        
        # middle zone: y=30%-60%
        mid_start = int(sh * 0.30)
        mid_end = int(sh * 0.60)
        mid_w = float(row_w[mid_start:mid_end].mean()) if mid_end > mid_start else 0
        mid_peak = mid_w / peak_w
        
        # lower zone: y=50%-bottom
        lower_start = sh // 2
        lower_w = float(row_w[lower_start:].mean()) if lower_start < sh else 0
        lower_peak = lower_w / peak_w
        
        # 分类决策树
        if color == "red":
            # ♥: middle保持宽 mid/peak>0.60 | ♦: middle窄
            if mid_peak > 0.60:
                key = "h"
            else:
                key = "d"
        else:  # black
            # ♣: lower宽 lower/peak>0.50 | ♠: lower窄
            if lower_peak > 0.50:
                key = "c"
            elif mid_peak > 0.40:
                # 中间地带: 用mid/peak再区分
                key = "c" if mid_peak > 0.45 else "s"
            else:
                key = "s"
        ...
        print(f"  {name}: {color} peak={peak_w} mid/peak={mid_peak:.2f} lower/peak={lower_peak:.2f} → {sym[key]} {mark}")
...
print(f"\n★★★ 新={n}/7 旧={o}/7 综合={n+o}/14 ({(n+o)/14*100:.0f}%) ★★★")
