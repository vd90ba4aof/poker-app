...
const ctx = {
  ...
  document:{
    ...
    createElement:()=>({style:{},appendChild:()=>{},setAttribute:()=>{},addEventListener:()=>{},querySelectorAll:()=>[]}),
    addEventListener:()=>{},body:{appendChild:()=>{}},readyState:'complete',
    createTextNode:()=>({}),head:{appendChild:()=>{}}
  },
  window:{addEventListener:()=>{},location:{href:''},setTimeout,setInterval:()=>{}},
...
// Eval in context - functions will be in ctx
try { vm.runInContext(blk1, ctx, {timeout:5000}); } catch(e) { /* API_BASE */ }
try { vm.runInContext(blk2, ctx, {timeout:5000}); } catch(e) { /* API_BASE */ }

// Now run the test in the same context
const testCode = `
let pass=0,fail=0;
...
var fnList=['decidePreflop','decidePostflop','postF','mcVsRange','riverExactEquity','multiPlayerEquity','postflopIP','handClassify','boardTexture','getOppRange','calcSPR','eH','handKeyToCards','getBlockerScore'];
for(var fi=0;fi<fnList.length;fi++){
  var n=fnList[fi];
  var exists=typeof this[n]==='function';
  ok(exists,n+' 不可用');
  console.log('  '+(exists?'✅':'❌')+' '+n+': '+typeof this[n]);
}
...
function tPF(label,hole,pos,act,stk,bet,raise,raiserPos){
  ...
  try{var r=decidePreflop(G);
    console.log('  '+label+': a='+((r&&r.a)||'null')+' r="'+(((r&&r.r)||'').substring(0,50))+'" eq='+(r&&r.eq||'-')+' c='+(r&&r.c||'-'));
    ok(r&&r.a,label+' 无决策');
...
function mkPost(hole,comm,pos,act,pot,bet,stk,scene){
  G={hole:pc(hole),comm:comm.map(function(c){return c?mc(c[0],c[1]):null;}),pot:pot||10,bet:bet||0,stk:stk||100,
    pos:pos,act:act||2,players:act||2,scene:scene||'check',
...
function tPost(label,hole,comm,pos,act,pot,bet,stk,scene){
  ...
  try{var r=decidePostflop(G);
    var eqs=(r&&r.eq!==undefined)?r.eq.toFixed(1):'-';
    var sprs=(r&&r.spr)?r.spr.toFixed(1):'-';
    console.log('  '+label+': a='+(r&&r.a||'null')+' eq='+eqs+'% scene='+(r&&r.scene||'-')+' spr='+sprs+' r="'+(((r&&r.r)||'').substring(0,45))+'"');
    ok(r&&r.a,label+' 无决策');
...
function tIP(label,pos,expect){
  ...
  try{var r=postF('AA');
    ...
    }else{console.log('  '+label+': no ip field (result='+JSON.stringify(r).substring(0,80)+')');}
...
ok(Math.abs(re1.eq-m1.eq)<15,'HU diff<15%');
...
ok(Math.abs(re2.eq-m2.eq)<15,'3way diff<15%');
...
// S8: handClassify
console.log('\\n── S8: handClassify ──');
var hct=[['AsAh','7s3h2c'],['AsKs','Kh7d2c'],['AsKs','AhKd2c'],['JdTs','Qh9d2c']];
for(var hi=0;hi<hct.length;hi++){
  var r=handClassify(pc(hct[hi][0]),pc(hct[hi][1]));
  console.log('  '+hct[hi][0]+'@'+hct[hi][1]+': '+(r?r.name:'null'));
  ok(r!==null,hct[hi][0]+'@'+hct[hi][1]+' null');
}

// S9: boardTexture
console.log('\\n── S9: boardTexture ──');
var btt=['Js9h2c','KsQsJs','7h7d2c','Th9h8h'];
for(var bi=0;bi<btt.length;bi++){
  var r=boardTexture(pc(btt[bi]));
  console.log('  '+btt[bi]+': wet='+r.wetness+' paired='+(r.paired||false));
  ok(r!==null,btt[bi]+' null');
}
