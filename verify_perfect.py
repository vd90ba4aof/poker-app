...
def test_all(PATH, test_cards, label):
    ...
    for name, x1, y1, x2, y2, is_hand, exp in test_cards:
        ...
        # ★★★ 关键修复: rank搜索限制在上半区(前50%) ★★★
        search_end = sh // 2
        peak_w = int(row_w[:search_end].max())
        peak_y = int(np.argmax(row_w[:search_end]))
        
        # rank end: 连续3行<50%peak
        ...
        print(f"  {name}: {color} peak={peak_w}@y{peak_y/sh*100:.0f}% rankEnd@y{rank_end/sh*100:.0f}% m@y{measure_y/sh*100:.0f}% w={suit_w}{flag} → {sym[key]} {mark}")
