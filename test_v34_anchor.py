"""
V3.4 锚定法测试 V2 — 使用正确的花色子区域（排除 rank indicator）
"""
import os
...
HAND_X_BASE = [(85, 180), (180, 295)]
HAND_Y_BASE = (1760, 2000)

# 花色子区域：排除上半 rank indicator（约 35%）
# 花色分析范围：y 从 35% 到 90%
...
def anchor_classify(mask, row_widths, reg_w, reg_h, is_red):
    ...
    if is_red:
        diamond_score = 0.0
        if 0.50 < wp < 0.80: diamond_score += 4.0
        if 0.40 < com_y < 0.62: diamond_score += 1.5
        if total_px > 0 and abs(ts - bs) / total_px < 0.35: diamond_score += 1.0
...
def analyze_suit(pixels, w, h, x1, y1, x2, y2):
    """对花色子区域（排除 rank）执行 V3.4 锚定法"""
    card_h = y2 - y1
    card_w = x2 - x1

    # 花色子区域：排除上半 rank indicator
    suit_y1 = y1 + int(card_h * SUIT_Y_START_FRAC)
    suit_y2 = y1 + int(card_h * SUIT_Y_END_FRAC)

    # 颜色判定用整牌区域（因为小 indicator 也反映颜色）
    ...
    suit = anchor_classify(mask, row_widths, reg_w, reg_h, is_red)
...
for filename, cards, note in TEST_SCREENS:
    ...
    hand_cards = cards[:2]; community_cards = cards[2:]

    for i, (rank, expected_suit) in enumerate(hand_cards):
        if expected_suit == "?": continue
        total += 1
        bx1, bx2 = HAND_X_BASE[i]; by1, by2 = HAND_Y_BASE
        ...
        details.append((note, f"手牌{i+1}", f"{rank}{suit}", f"{rank}{expected_suit}", ok))
        print(f"  {note} 手牌{i+1}: {rank}{suit} vs {rank}{expected_suit} {ok} (red={rpx},black={bpx},isRed={is_red})")

    for i, (rank, expected_suit) in enumerate(community_cards):
        if expected_suit == "?": continue
        total += 1
