...
def compute_top_indentation(mask, row_widths, reg_w, reg_h):
    """
    检测顶部凹陷：♥ 顶部有凹陷（窄→宽），♦ 顶部直接到最宽
    返回凹陷分数：越高越像 ♥
    """
    # 找到最宽行
    max_width = max(row_widths)
    if max_width == 0: return 0.0
    ...
    if widest_y < 3: return 0.0  # 最宽行太靠上，无法判断

    # 计算顶部 1/3 区域的平均宽度
    top_third_end = widest_y
    if top_third_end < 3: return 0.0

    top_avg = sum(row_widths[:top_third_end]) / top_third_end

    # 凹陷分数 = (最宽 - 顶部平均) / 最宽
    # ♥ 的顶部平均远小于最宽（凹陷明显）
    # ♦ 的顶部平均接近最宽（无凹陷）
    indentation = (max_width - top_avg) / max_width
    return indentation
...
def classify_suit_v5(mask, row_widths, reg_w, reg_h, is_red):
    """V3.5 顶部凹陷检测"""
    ...
    top_indent = compute_top_indentation(mask, row_widths, reg_w, reg_h)

    if is_red:
        # ♥ HEART: 顶部凹陷明显
        # ♦ DIAMOND: 顶部无凹陷，直接到最宽
        heart_score = 0.0
        if top_indent > 0.35: heart_score += 4.0  # 凹陷明显
        elif top_indent > 0.25: heart_score += 2.0
...
def analyze_suit(pixels, w, h, x1, y1, x2, y2):
    ...
    suit = classify_suit_v5(mask, row_widths, reg_w, reg_h, is_red)
